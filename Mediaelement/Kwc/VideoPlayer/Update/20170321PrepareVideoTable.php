...
class Mediaelement_Kwc_VideoPlayer_Update_20170321PrepareVideoTable extends Kwf_Update
{
    public function update()
    {
        ...
        if (!isset($fields['video_width'])) {
            $db->query("ALTER TABLE `kwc_advanced_video_player` ADD `video_width` VARCHAR( 255 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER `mp4_kwf_upload_id`;");
        }
        ...
        if (!isset($fields['mp4_url'])) {
            $db->query("ALTER TABLE `kwc_advanced_video_player` ADD `mp4_url` TEXT NOT NULL AFTER `mp4_kwf_upload_id`;");
        }
        ...
        if (!isset($fields['size'])) {
            $db->query("ALTER TABLE `kwc_advanced_video_player` ADD `size` ENUM(  'contentWidth',  'userDefined' ) NOT NULL AFTER  `mp4_kwf_upload_id`;");
